...
struct tc_t {
  ...
  tc_ident_t prev, next;  // identifiers of the TCs running the previous and next chunks of
                          // threads; the values shouldn't be used on the first generation of the
                          // first TC in the family and on the last generation of the last TC in the family,
                          // respectively. To get a consistent value, code generated by the compiler should
                          // use the _get_prev_ident() and _get_next_ident() functions.
